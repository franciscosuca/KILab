...
```text
iiot-<NUMBER>-<NAME>/iiot-<NUMBER>-<NAME>.md
```

For plans without an issue number:

```text
iiot-XXXX-<inferred-name>/iiot-XXXX-<inferred-name>.md
```
...
Create iiot-2851-<inferred-name>/ under $PLAN_ROOT/drafts/, preserve the request in context/request.md, and create the canonical Markdown skeleton. Leave workflows/ and work-packages/ empty. Do not implement product code.
```
...
No issue number was provided. Use iiot-XXXX-<inferred-name>/, infer a lowercase name of at most four words, and do not guess a number. Preserve the request in context/request.md, create the canonical Markdown skeleton, leave workflows/ and work-packages/ empty, and do not implement product code.
```

### 3. Investigate and write the canonical plan

```text
/plan-write iiot-XXXX-dynamic-body-validation
...
```text
Create the workflow documents for the approved canonical plan iiot-XXXX-dynamic-body-validation. Store them under its workflows/ directory, tag them with the canonical issue ID, link them from Related documents, and do not create work packages unless separately requested.
```

### 5. Create work packages on request

```text
Create work packages for the approved canonical plan iiot-XXXX-dynamic-body-validation. Split the approved phases into wp-1-<short-description>.md files under work-packages/, begin each with the issue tag, title, and ## Goal, link them from Related documents, and do not add automatic Plan mapping or Source reference blocks.
```

### 6. Dispatch an approved plan

```text
/plan-dispatch iiot-XXXX-dynamic-body-validation
...
```text
/plan-sync iiot-XXXX-dynamic-body-validation
