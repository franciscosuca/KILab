...
5. Change canonical frontmatter to `open`, add the dispatch timestamp and agent to `Dispatch Log`, move the complete plan bundle to `$PLAN_ROOT/open/<plan-slug>/`, and update the index before launching work.

## Agent Contract

Launch an isolated implementation agent with `runSubagent`. Pass the canonical plan path, complete bundle path, target repository, branch, exact checks, non-goals, dirty-file warning, and this instruction: implement the plan, use supporting context/workflows/work-packages as guidance, validate after each focused edit, record deviations, and never merge.
...
find "$PLAN_ROOT/next/<plan-slug>" -maxdepth 3 -type f -print | sort
...
Append branch, commit SHAs, draft review URL, command results, CI state, review state, deviations, and blockers to `Dispatch Log`. Keep the complete bundle in `open`; only `/plan-sync` decides whether it is done.