...
check test -f "$CATALOG/adapters/pi/APPEND_SYSTEM.md"
check grep -Fq 'When I write "kyas"(short for - keep your answer short) in my prompt Keep answers concise—usually to maximum 30 words. Give only the detail needed to answer; expand when I ask or when it’s necessary for accuracy.' "$CATALOG/adapters/pi/APPEND_SYSTEM.md"
check grep -Fq 'When I write "kyas"(short for - keep your answer short) in my prompt Keep answers concise—usually to maximum 30 words. Give only the detail needed to answer; expand when I ask or when it’s necessary for accuracy.' "$CATALOG/adapters/claude/CLAUDE.md"
check grep -Fq 'When I write "kyas"(short for - keep your answer short) in my prompt Keep answers concise—usually to maximum 30 words. Give only the detail needed to answer; expand when I ask or when it’s necessary for accuracy.' "$CATALOG/adapters/copilot/copilot-instructions.md"
check grep -Fq 'copy_file "$CATALOG/adapters/pi/APPEND_SYSTEM.md" "$DEST/APPEND_SYSTEM.md"' "$SCRIPT_DIR/install-pack.sh"
