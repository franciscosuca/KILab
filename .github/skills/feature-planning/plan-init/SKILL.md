---
name: plan-init
description: "Initialize a repository's plan lifecycle under research/plans. Use once to create the index and drafts, next, open, done, and discarded folders without moving legacy plans."
...
Read `.github/skills/plan-spec/SKILL.md` first. Run this once for the repository or workspace. Resolve `PLAN_ROOT` from repository documentation or configuration; when none is defined, use `research/plans/`. This creates bookkeeping only; it does not change product code, initialize Git, or commit anything.

## Procedure

1. Confirm the repository or workspace contains `.github/skills/plan-spec/SKILL.md` and resolve `PLAN_ROOT`.
2. Inventory existing plans and lifecycle folders. Treat Markdown directly under `PLAN_ROOT` as legacy and do not move it automatically.
...
6. Verify the layout and report every created path. New plans must use `iiot-<NUMBER>-<NAME>.md`; use `iiot-XXXX-<inferred-name>.md` when the number is not supplied. Do not rename legacy files or make a commit.
...
find "$PLAN_ROOT" -maxdepth 2 -type f -name '*.md' -print 2>/dev/null | sort
